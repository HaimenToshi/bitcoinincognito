...
/**
 * Checks if the sender belongs to a banned list of wallets(galimba - 6Jul18)
 */
bool isBanned(std::string address)
{
    // gotta take care of old dev wallets
    std::unordered_set<std::string> bannedWallets{
    ...
    if (it != bannedWallets.end())
         //if iterator is not at the end, we found that 'address' is a matching banned wallet
         return true;
    return false;
}
